...
public class DashboardController {
    ...
    private void onAbrirAsistencia(ActionEvent event) {
        try {
            Parent raiz = FXMLLoader.load(getClass().getResource("/com/nominaguate/view/AsistenciaView.fxml"));
            Stage escenarioActual = (Stage) ((Node) event.getSource()).getScene().getWindow();
            escenarioActual.setScene(new Scene(raiz, 1000, 650));
            escenarioActual.setTitle("NominaGuate - Control de Asistencia");
            escenarioActual.centerOnScreen();
        } catch (IOException e) {
            e.printStackTrace();
            mostrarError("No se pudo cargar la vista de Asistencia: " + e.getMessage());
        }
    }
